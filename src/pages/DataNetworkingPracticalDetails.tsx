...
const DataNetworkingPracticalDetails = () => {
  const { id } = useParams<{ id: string }>();
  const practicalId = parseInt(id || '1');
  const practical = networking.find(p => p.id === practicalId);
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-16 text-center">
          ...
          <Link to="/dcn">
            <Button>Go back to Data Communication Networking</Button>
          </Link>
        </div>
      </div>
    );
  ...
  const prevPractical = practicalId > 1 ? 
    networking.find(p => p.id === practicalId - 1) : null;
    
  const nextPractical = practicalId < networking.length ? 
    networking.find(p => p.id === practicalId + 1) : null;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-4 flex justify-between items-center">
          <Link to="/dcn" className="inline-flex items-center text-primary hover:underline">
            <ChevronLeft size={16} />
...
export default DataNetworkingPracticalDetails;
